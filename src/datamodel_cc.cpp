...
QVariant DataModel::data(
        ...
        case Qt::DisplayRole: // Данные для отображения
                  ...
                  str = QString("YN%1").arg(col);
                  if (map.contains(str)) {
                      QStringList ss = map.value(str).toString().split('@');
                      return value.toInt() != 0 ? ss.at(1) : ss.at(0);
                  }
        ...
        case Qt::TextColorRole: {// Цвет текста
            ...
            if (map.contains(QString("YN%1").arg(col))) {
                QColor color;
                QStringList cc = map.value(str).toString().split('@');
                if (QSqlQueryModel::data(index, Qt::DisplayRole).toInt() != 0)
                    color.setNamedColor(cc.at(1));
                else color.setNamedColor(cc.at(0));
